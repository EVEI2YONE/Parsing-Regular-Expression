...
my_LexicalAnalyzer::my_LexicalAnalyzer(Track* l, std::string s) {
    ...
    this->inputText = s;
    sets_head = NULL;
    my_getToken(s, 0);	//<-------MY_GETTOKEN FUNCTION CALLED
    free_set(sets_head);
    sets_head = NULL;
    shparser = NULL;
}
...
void my_LexicalAnalyzer::my_getToken(std::string s, int p) {
    ...
    */
    ...
    my_getToken(str_temp, 0);	//<--------------------MY_GETTOKEN FUNCTION CALLED
    return;
}
...
void my_LexicalAnalyzer::match(REG *r, string s, int p) {
    ...
    */
    ...
    temp1 = match_one_char(object, '_');//<--------------------MATCH_ONE_CHAR FUNCTION CALLED
